  ...
  return (
    <DialogPortal data-slot="dialog-portal">
      ...
      <DialogPrimitive.Content
        ...
        className={cn(
            "bg-background data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed left-1/2 top-4 sm:top-1/2 sm:translate-y-[-50%] z-50 grid w-full max-w-[calc(100%-2rem)] translate-x-[-50%] gap-4 rounded-lg border p-4 shadow-lg duration-200 sm:max-w-lg",
            className
          )}
        {...props}
      >
        <div className="w-full overflow-auto max-h-[calc(100vh-4rem)]">{children}</div>
        {showCloseButton && (
