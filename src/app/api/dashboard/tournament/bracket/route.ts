  ...
  try {
    ...
    if (games.length > 0) {
      ...
      const calculateGameImportance = (game: any, maxRound: number) => {
        ...
        // (Vereinfacht: Annahme, dass höhere Spieler-IDs = bessere Spieler sind)
        if (game.player1 && game.player2) {
          const player1Rank = game.player1.id; // Vereinfacht
          const player2Rank = game.player2.id; // Vereinfacht
          const rankDiff = Math.abs(player1Rank - player2Rank);
          importance += Math.min(rankDiff * 10, 50); // Max 50 Punkte für Ranking-Unterschied
        }
      ...
      const otherBoards = availableBoards.filter(board => !board.isMain || board.games.length > 0);

      let assignedCount = 0;

      // 1. Weise das wichtigste Spiel der Hauptscheibe zu (falls verfügbar)
      if (mainBoard && sortedGames.length > 0) {
      ...
      for (const board of otherBoards) {
        if (board.games.length === 0 && assignedCount < readyGames.length && sortedGames.length > 0) {
          await prisma.game.update({
            where: { id: sortedGames[0].id },
